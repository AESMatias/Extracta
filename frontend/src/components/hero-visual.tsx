...
export function HeroVisual() {
  return (
    <div className="relative mx-auto w-full max-w-md lg:max-w-none" aria-hidden>
      <div className="absolute -inset-8 -z-10 bg-gradient-to-tr from-brand-500/30 via-cyan-400/15 to-emerald-400/25 blur-3xl" />

      {/* The source PDF, with a line scanning it the way the AI reads it */}
      <div className="relative w-[78%] rotate-[-4deg] overflow-hidden border border-slate-200 bg-white p-5 shadow-xl shadow-slate-900/10 dark:border-slate-700 dark:bg-slate-800">
        <span
          className="animate-scan pointer-events-none absolute inset-x-0 top-14 h-px bg-emerald-400 shadow-[0_0_14px_3px_rgb(52_211_153/0.55)]"
          style={{ "--scan-distance": "110px" } as CSSProperties}
        />
        <div className="flex items-center gap-2">
          <div className="grid size-8 place-items-center bg-rose-100 text-rose-600 dark:bg-rose-500/15">
        ...
        <div className="mt-4 space-y-2">
          <div className="h-2.5 w-2/3 bg-slate-200 dark:bg-slate-700" />
          <div className="h-2 w-1/2 bg-slate-100 dark:bg-slate-700/70" />
          <div className="mt-4 grid grid-cols-3 gap-2">
            {Array.from({ length: 9 }).map((_, i) => (
              <div key={i} className="h-2 bg-slate-100 dark:bg-slate-700/70" />
            ))}
          </div>
          <div className="mt-4 ml-auto h-3 w-1/3 bg-brand-100 dark:bg-brand-500/20" />
        </div>
      </div>

      {/* The extracted data */}
      <div className="glass relative -mt-16 ml-auto w-[82%] animate-float p-5 shadow-2xl shadow-brand-900/15">
