...
const variants: Record<Variant, string> = {
  primary:
    "bg-gradient-to-r from-brand-600 via-violet-600 to-fuchsia-600 text-white shadow-lg shadow-brand-600/25 hover:shadow-xl hover:shadow-brand-600/30 hover:brightness-110",
  secondary:
...
export function Card({ className, children, ...props }: ComponentProps<"div">) {
  return (
    <div
      className={clsx(
        "rounded-3xl border border-slate-200/80 bg-white shadow-sm shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900/70 dark:shadow-none",
        className,
      )}
      {...props}
    >
...
export function SectionTitle({ eyebrow, title, subtitle, center }: { eyebrow?: string; title: ReactNode; subtitle?: ReactNode; center?: boolean }) {
  return (
    <div className={clsx("max-w-2xl", center && "mx-auto text-center")}>
      {eyebrow && <p className="text-sm font-semibold tracking-wide text-brand-600 uppercase dark:text-brand-400">{eyebrow}</p>}
  ...
  return (
    <div className={className}>
      ...
      <input
        id={inputId}
        className={clsx(
          "block h-11 w-full rounded-xl border bg-white px-3.5 text-[15px] text-slate-900 shadow-sm transition outline-none placeholder:text-slate-400 focus:ring-4 dark:bg-slate-900 dark:text-white",
          error
...
export function Alert({ tone = "red", title, children, action }: { tone?: "red" | "amber" | "green" | "brand"; title?: string; children?: ReactNode; action?: ReactNode }) {
  ...
  return (
    <div role={tone === "red" ? "alert" : "status"} className={clsx("flex flex-col gap-3 rounded-2xl border px-4 py-3 text-sm sm:flex-row sm:items-center", styles)}>
...
export function Progress({ value, className, tone = "brand" }: { value: number; className?: string; tone?: "brand" | "amber" | "red" }) {
  const bar = { brand: "from-brand-500 to-fuchsia-500", amber: "from-amber-400 to-orange-500", red: "from-rose-500 to-red-600" }[tone];
  return (
    <div className={clsx("h-2 w-full overflow-hidden rounded-full bg-slate-200/80 dark:bg-slate-800", className)}>
      <div
        className={clsx("h-full rounded-full bg-gradient-to-r transition-[width] duration-500 ease-out", bar)}
